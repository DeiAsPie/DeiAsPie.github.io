...
// Configuration
const CONTENT_DIRS = ["content", "layouts"];
  ...
  for (let i = 1; i < headings.length; i++) {
    ...
    if (current.level > previous.level + 1) {
      issues.push({
        type: "heading",
        severity: "warning",
        message: `Heading level jump from H${previous.level} to H${
          current.level
        } at line ${current.line}. Consider using H${
          previous.level + 1
        } instead.`,
        line: current.line,
      });
...
function checkImageAttributes(content) {
  ...
  while ((match = IMAGE_PATTERN.exec(content)) !== null) {
    ...
    // Check for loading attribute presence (not just lazy)
    if (!imgTag.match(/loading=["'][^"']+["']/i)) {
      issues.push({
        type: "performance",
        severity: "warning",
        message: 'Image missing loading attribute (e.g., loading="lazy")',
        line: line,
        context: imgTag.substring(0, 50) + "...",
      });
    }
...
function checkLoremIpsum(content) {
  ...
  while ((match = LOREM_IPSUM_PATTERN.exec(content)) !== null) {
    ...
    issues.push({
      ...
      line: line,
      context:
        "... " + content.substring(match.index - 10, match.index + 20) + " ...",
    });
  }
